...
class Retrieve:
    ...
    def forQuery(self, query):
        ...
        tr_candidate, tr_candidate_id = self.getCandidate(query) 
        output = None
        ...
        if self.termWeighting == 'binary':
            ...
            output =  self.tf_model(tr_candidate, query)
        elif self.termWeighting == 'tfidf':
            output =  self.tfidf_model(tr_candidate, query)
        del tr_candidate, tr_candidate_id
        return output      
    ...
    def getCandidate(self, query):
        ...
        for each_iterm in set(queryIterm):            
            if each_iterm in self.index:   # If the target document includes the query key we should put the id into a document
                ...
                for doc_id,value in temp.items():
                    get_candidate_id.append(doc_id)
        return get_candidate,get_candidate_id
    ...
    def tf_model(self, all_candidate, query):
        Results = dict()
        tr_id_termcount = dict()
        ...
        score = 0
        for term in query:
            if term in all_candidate:
                for ID,count in all_candidate[term].items():
                    value_sum = count * query[term]
                    if ID in tr_id_termcount:
                        tr_id_termcount[ID] += value_sum
                    else:
                        tr_id_termcount[ID] = value_sum
        for ID,value in tr_id_termcount.items():
            Results[ID] = value / math.sqrt(self.doc_len[ID])
